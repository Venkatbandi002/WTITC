...
const RocketIcon = ({ className = 'w-6 h-6' }) => (
  <svg xmlns="http://www.w3.org/2000/svg" className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
    <path d="M4.5 16.5l-3 3 1.5 1.5 3-3L18 6l-3-3L4.5 16.5z" />
    <path d="M14 6l-3-3M15 9l-3-3M18 12l-3-3" />
  </svg>
);
...
const LightbulbIcon = ({ className = 'w-6 h-6' }) => (
  <svg xmlns="http://www.w3.org/2000/svg" className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
    ...
    <path d="M12 21v-3M19 14.5a7 7 0 1 0-14 0c0 3.33 2.5 4.9 2.5 4.9h9s2.5-1.57 2.5-4.9z" />
  </svg>
);

const HandshakeIcon = ({ className = 'w-6 h-6' }) => (
  <svg xmlns="http://www.w3.org/2000/svg" className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
    <path d="M10 16.5l2 2 4-4M8 8a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2l-4 4-4-4V8zM18 10l-2 2-2-2M6 10l2 2 2-2" />
  </svg>
);
...
const SectionItem: React.FC<{ section: InnovationSection }> = ({ section }) => {
  ...
  return (
    <div className="flex flex-col sm:flex-row items-start py-6 border-b border-gray-100 last:border-0">
      <div className={`p-4 ${section.color}/10 rounded-xl ${textColor} flex-shrink-0 sm:mr-6 mb-4 sm:mb-0`}>
        <Icon className="w-8 h-8" />
      </div>
      <div>
        <h3 className="text-2xl font-bold text-gray-900">{section.title}</h3>
        <p className={`text-md ${textColor} font-medium mt-1 mb-3`}>{section.subtitle}</p>
        <p className="text-gray-700 mb-4">{section.description}</p>
        <Link
          to={section.link}
          className="inline-flex items-center px-4 py-2 text-sm font-medium rounded-lg shadow-md text-white bg-blue-600 hover:bg-blue-700"
        >
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-blue-600/90 to-blue-600/0 flex items-center">
          <div className="px-6">
            ...
            <p className="text-xl text-white/90 mt-2">
              The framework for accelerating global technology and R&D strategy.
            </p>
          </div>
        </div>
      </section>
